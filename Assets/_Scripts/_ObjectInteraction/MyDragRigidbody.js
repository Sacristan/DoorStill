...
function Update()
{
	...
	else
	{
		...
		var hitt : RaycastHit;
		if( Physics.Raycast(mainCamera.ScreenPointToRay(cursorCtrlScript.cursorPos),  hitt, 100 ) ) {
	...
	// We need to actually hit an object
	var hit : RaycastHit;
	if (!Physics.Raycast(mainCamera.ScreenPointToRay(cursorCtrlScript.cursorPos),  hit, 100)) {
		return;
	}
	...
	DragObject(hit.distance, hit.point, mainCamera.ScreenPointToRay(cursorCtrlScript.cursorPos).direction);
}

function DragObject (distance : float, hitpoint : Vector3, dir : Vector3)
{
	if(!GameObject.FindWithTag("3DGUI"))
	{
		...
		{
			var ray = mainCamera.ScreenPointToRay (cursorCtrlScript.cursorPos);
			springJoint.transform.position = ray.GetPoint(distance);
			yield;
		}
		//PASKATIITIES
		if (Mathf.Abs(mousePos.x - cursorCtrlScript.cursorPos.x) <= 2 && Mathf.Abs(mousePos.y - cursorCtrlScript.cursorPos.y) <= 2 && Time.time - startTime < .2 && springJoint.connectedBody)
		{
			dir.y = 0;
			dir.Normalize();
...
function OnPostRender()
{
	...
	{
		if(!GameObject.FindWithTag("3DGUI") && (go.transform.localScale==Vector3(1,1,1))) Graphics.DrawMeshNow( m.sharedMesh, m.transform.position, m.transform.rotation );
	}
}

function Start()
{
	cursorCtrlScript = GameObject.FindWithTag("ScriptGO").GetComponent(CursorCtrl) as CursorCtrl;
}
