...
public class PhotonViewInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (sceneProp.isInstantiatedPrefab && !sceneProp.prefabOverride)
        {
            ...
            GameObject pPrefab2 = (EditorUtility.GetPrefabParent(mp.gameObject) as GameObject);
            if (pPrefab2 != null)
            {
        ...
        else
        {
            ...
            if (mp.viewID.ID != 0 || mp.isSceneView)
            {
                ...
                mp.viewID = new PhotonViewID(0, null);
                mp.SetSceneID(0);
    ...
    static void SetViewID(PhotonView mp, int ID)
    {
        ...
        if (!EditorApplication.isPlaying)
        {
            ...
            GameObject pPrefab = (EditorUtility.GetPrefabParent(mp.gameObject) as GameObject);
            if (pPrefab != null)
            {
    ...
    static void VerifyAllSceneViews()
    {
        PhotonView[] photonViews = Resources.FindObjectsOfTypeAll(typeof(PhotonView)) as PhotonView[];
        foreach (PhotonView view in photonViews)
        {
            if (view.isSceneView && !IsSceneViewIDFree(view.viewID.ID, view))
            {
