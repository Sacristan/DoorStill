...
public class NetworkingPeer : PhotonPeer, IPhotonPeerListener
{
    ...
    /// </summary>
    internal RoomToGetInto mRoomToGetInto { get; set; }
    ...
    private Dictionary<PhotonPlayer, int> lastCachedEventPerPlayer = new Dictionary<PhotonPlayer, int>();
    ...
    {
        if (this.PeerState != PeerStateValue.Connected)
        {
    ...
    private void LeftRoomCleanup()
    {
        ...
        this.lastCachedEventPerPlayer = new Dictionary<PhotonPlayer, int>();
    ...
    public bool OpCreateGame(string gameID, bool isVisible, bool isOpen)
    {
        return this.OpCreateGame(gameID, isVisible, isOpen, (byte)0, null);
    }

    public bool OpCreateGame(string gameID, bool isVisible, bool isOpen, byte maxPlayers)
    {
        return this.OpCreateGame(gameID, isVisible, isOpen, maxPlayers, null);
    }

    public bool OpCreateGame(string gameID, bool isVisible, bool isOpen, Hashtable customGameProperties)
    {
        return this.OpCreateGame(gameID, isVisible, isOpen, (byte)0, customGameProperties);
    }
    ...
    public bool OpCreateGame(string gameID, bool isVisible, bool isOpen, byte maxPlayers, Hashtable customGameProperties)
    {
        ...
        this.mRoomToGetInto = new RoomToGetInto(gameID, gameProperties, JoinType.CreateGame);

        Dictionary<byte, object> op = new Dictionary<byte, object>();
        op[ParameterCode.GameProperties] = gameProperties;
        op[ParameterCode.ActorProperties] = this.GetLocalActorProperties();
        ...
        //Debug.Log(OperationCode.CreateGame + ": " + SupportClass.DictionaryToString(op));
    ...
    public bool OpJoin(string gameID)
    {
        ...
        this.mRoomToGetInto = new RoomToGetInto(gameID, null, JoinType.JoinGame);
        ...
        // Debug.Log(OperationCode.JoinGame + ": " + SupportClass.DictionaryToString(op));
    ...
    public bool OpJoinRandom(Hashtable expectedGameProperties)
    {
        this.Listener.DebugReturn(DebugLevel.INFO, "OpJoinRandom()");

        this.mRoomToGetInto = new RoomToGetInto(null, expectedGameProperties, JoinType.JoinRandomGame);

        Dictionary<byte, object> op = new Dictionary<byte, object>();
        if (expectedGameProperties != null && expectedGameProperties.Count > 0)
        {
            op[ParameterCode.Properties] = expectedGameProperties.StripToStringKeys();
        }
    ...
    private bool OpSetPropertiesOfActor(int actorNr, Hashtable actorProperties, bool broadcast, byte channelId)
    {
    ...
    protected bool OpAuthenticate()
    {
        this.Listener.DebugReturn(DebugLevel.INFO, "OpAuthenticate()");

        Dictionary<byte, object> opParameters = new Dictionary<byte, object>();
        opParameters[ParameterCode.AppVersion] = this.mAppVersion;
        opParameters[ParameterCode.ApplicationId] = this.mAppId;

        return this.OpCustom(OperationCode.Authenticate, opParameters, true, (byte)0, false);
    }

    protected void OpSetProperty(byte propCode, object value)
    {
    ...
    public virtual bool OpRaiseEvent(byte eventCode, Hashtable evData, bool sendReliable, byte channelId, EventCaching cache, ReceiverGroup receivers)
    {
        ...
        {
            this.Listener.DebugReturn(DebugLevel.INFO, "OpRaiseEvent() (w/ caching)");
        }
    ...
    private void AddNewPlayer(int ID, PhotonPlayer player)
    {
        ...
        {
            this.lastCachedEventPerPlayer.Add(player, -1);
    ...
    private void HandleEventLeave(int actorID)
    {
        ...
        this.lastCachedEventPerPlayer.Remove(player);
    ...
    public void ChangeLocalID(int newID)
    {
        ...
        if (!this.lastCachedEventPerPlayer.ContainsKey(this.mLocalActor))
        {
            this.lastCachedEventPerPlayer.Add(this.mLocalActor, -1);
        }
        else
        {
            this.lastCachedEventPerPlayer[this.mLocalActor] = -1;
        }
    ...
    public void OnOperationResponse(OperationResponse operationResponse)
    {
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.LeaveLobby:
                ...
                // this.mListener.leaveLobbyReturn();
                break;
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            ...
            case EventCode.SetProperties:
                ...
                // RemoveByteTypedPropertyKeys(gameProperties, false);
                // RemoveByteTypedPropertyKeys(actorProps, false);
                // in case, we have received the event for a standard property,
                // not a custom one, both dictionaries will be empty now and we
                // should not call the application's eventAction-methods at all
                // if (gameProperties != null && gameProperties.Count > 0)
                // {
                // this.mListener.propertiesOfGameEventAction(gameProperties);
                // }
                // if (actorProps != null && actorProps.Count > 0)
                // {
                // this.mListener.propertiesOfActorEventAction(targetActorNr, actorProps);
                // }
                break;
            ...
            case PhotonNetworkMessages.CloseConnection:
                ...
                {
                    Debug.LogError(
                        "Error: Someone else(" + orgPlayer + ") then the masterserver requests a disconnect!");
                }
                else
                {
            ...
            case PhotonNetworkMessages.Destroy:
                ...
                {
                    Debug.LogError(
                        "ERROR: Illegal destroy request on view ID=" + viewID + " from player/actorNr: " + actorNr);
                }
                else
                {
    ...
    private void RemoveInstantiatedGO(GameObject go)
    {
        if (go == null)
        {
        ...
        if (instantiateId == -1)
        {
            if (DebugOut == DebugLevel.ERROR)
            {
                this.DebugReturn(DebugLevel.ERROR, "Can't find GO in instantiation list. Destroying: " + go);
            }
        }
        else
        {
        ...
        foreach (KeyValuePair<int, PhotonView> kvp in this.photonViewList)
        {
            ...
            if (view.observed != null && view.synchronization != ViewSynchronization.Off)
            {
                ...
                if (view.owner == this.mLocalActor || (view.isSceneView && this.mMasterClient == this.mLocalActor))
                {
                    ...
                    {
                        if (!this.SameOnSerializeHashtable(evData, view.lastOnSerializeDataSent))
                        {
    ...
    private bool SameOnSerializeHashtable(Hashtable one, Hashtable two)
    {
        ...
        Hashtable dataOne = (Hashtable)one[(byte)1];
        Hashtable dataTwo = (Hashtable)two[(byte)1];
    ...
    private Hashtable WriteSerializeData(PhotonView view)
    {
        ...
        if (typeof(MonoBehaviour).IsInstanceOfType(view.observed))
        {
            ...
            PhotonStream pStream = new PhotonStream(true, contents);
            ...
            evData[(byte)1] = pStream.ToHashtable();
        }
    ...
    private void ReadOnSerialize(Hashtable data, PhotonPlayer sender)
    {
        ...
        Hashtable contents = (Hashtable)data[(byte)1];
    ...
    private void ExecuteOnSerialize(MonoBehaviour monob, PhotonStream pStream, PhotonMessageInfo info)
    {
        ...
        {
            Debug.LogError(
                "Tried to run " + ReflectionMethods.OnPhotonSerializeView + ", but this method was missing on: " + monob);
        }
    }
