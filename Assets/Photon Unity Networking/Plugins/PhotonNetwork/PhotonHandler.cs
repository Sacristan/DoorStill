...
public class PhotonHandler : Photon.MonoBehaviour, IPhotonPeerListener
{
    ...
    {
        if (PhotonNetwork.logLevel == NetworkLogLevel.Full
            ||
            (PhotonNetwork.logLevel == NetworkLogLevel.Informational
             && (level == DebugLevel.ERROR || level == DebugLevel.WARNING || level == DebugLevel.INFO))
            || (level == DebugLevel.ERROR || level == DebugLevel.WARNING))
        {
